...
describe("rule-based failure diagnosis", () => {
  ...
  it("does not describe a valid passing trial as a recorded failure", () => {
    const diagnosis = diagnose(
      [
        event(1, "file.read.completed", { path: "src/a.ts" }),
        event(2, "edit.completed", { path: "src/a.ts", changedLines: 1 }),
      ],
      result({
        outcome: "passed",
        hardGates: [
          { id: "setup", status: "passed", evidence: ["grade.json"] },
          { id: "verifier", status: "passed", evidence: ["grade.json"] },
          { id: "assertions", status: "passed", evidence: ["grade.json"] },
        ],
      }),
    );
    expect(diagnosis.findings).toEqual([]);
    expect(diagnosis.deterministicFacts).toMatchObject({
      integrity: "valid",
      outcome: "passed",
    });
    expect(diagnosis.limitations).toContain(
      "trial_passed_no_failure_to_diagnose",
    );
  });
