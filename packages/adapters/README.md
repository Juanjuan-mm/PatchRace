...
Versioned, local-first adapters for Pi, Claude Code, and Codex. The package implements the `1.0.0` adapter contract: executable/version/auth probes, pure invocation preparation, raw-first structured streaming, M3 process-group cancellation, normalized errors and metrics, observable trace mapping, compatibility checks, and explicit redacted export.
...
`writeOtlpJsonTraceExport` produces a local OpenTelemetry OTLP/JSON `resourceSpans` document. It requires `optIn: "confirmed"`, applies the M3 `Redactor` to the full export before writing, creates the destination with `wx`, preserves missing timestamps as missing, and never uploads or publishes. Raw vendor streams are excluded.
