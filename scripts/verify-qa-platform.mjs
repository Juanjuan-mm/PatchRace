...
function validateEnvironment() {
  ...
  if (!["darwin", "linux"].includes(operatingSystem)) {
    throw new Error(
      `QA-02 supports only macOS and Linux; received '${operatingSystem}'.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `QA-02 requires Node >=22.22.0 on major 22 or 24; received ${nodeVersion}.`,
    );
  }
  ...
  if (compareVersions(parseVersion(gitVersion, "Git"), [2, 39, 0]) < 0) {
    throw new Error(`QA-02 requires Git >=2.39.0; received ${gitVersion}.`);
  }
  ...
  if (pnpmVersion !== "10.34.5") {
    throw new Error(
      `QA-02 requires the pinned pnpm 10.34.5; received ${pnpmVersion}.`,
    );
  }
