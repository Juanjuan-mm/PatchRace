...
Human CLI progress and diagnostics go to stderr. Requested `--json` output is one stable JSON value on stdout. M2 routes commands only to side-effect-free placeholder services; later milestones replace those services without moving behavior into argument parsing.
