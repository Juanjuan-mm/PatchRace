...
Status: source-install instructions for the `v0.1.0-rc.1` GitHub preview  
Last verified: 2026-07-23

PatchRace `v0.1.0-rc.1` is a source-only GitHub preview. It is not published to
...
On the maintained reference machine, `DOC-01` verified a new source copy with
no `node_modules` or built output in 7.244 seconds: frozen dependency
installation took 1.343 seconds and the quickstart took 5.699 seconds. That is
one reference measurement, not a universal performance promise. The verifier
records install and first-valid-comparison time separately so a warm workspace
is not disguised as a clean install.
...
Live provider/paid dogfood is not part of this quickstart and requires an exact
task, endpoint/model, and budget authorization.
