...
Status: accepted for v0.1 by `ARC-08`  
Last updated: 2026-07-22

## Decision summary

PatchRace v0.1 will be a strict ESM TypeScript monorepo on Node.js LTS, distributed through npm, with pnpm workspaces, Vitest, JSON Schema/Ajv validation, and a dependency-light static HTML report built with vanilla TypeScript/CSS. Python is optional and out-of-process only. Agent integrations invoke official local CLIs; Pi additionally has a direct SDK path.

This choice is supported by all M1 spikes: the three launch agents expose structured headless streams, Pi's SDK loads in the same Node process, Git/process primitives are sufficient for the tested lifecycle, and no critical Python-only library is required.

## Runtime and language

- Runtime support: latest patched Node `22.x` and `24.x` LTS on supported macOS/Linux; engine range `>=22.22.0 <25` at M2 scaffold time.
...
packages/
  ...
  pi-extension    thin Pi-native entry over core APIs (later milestone)
fixtures/         deterministic repos/traces/adapter streams
```

Cycles are forbidden. `contracts` has no side effects and minimal dependencies; `core` does not import CLI/report; adapters do not grade; reports read stable artifacts. M2 may combine packages when a boundary creates overhead without independent contracts, but dependency direction remains.

## CLI, config, and validation

Baseline choices to confirm/pin during M2:
...
Exact supported CLI ranges are established by adapter compatibility tests later. M1 versions are evidence points, not promised ranges: Pi `0.81.1`, Claude Code `2.1.104`, Codex `0.145.0-alpha.18`; a broken global Codex `0.120.0` installation proves health probing is mandatory.
