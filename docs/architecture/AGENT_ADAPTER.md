...
Status: accepted for v0.1 by `ARC-05`  
...
- `base.ts` owns health/version/auth probing, pure path preparation, raw-first JSONL collection, M3 `runProcess` integration, lifecycle states, cancellation, and common error normalization.
...
The deterministic contract fixture represents all three structured protocols and completes the same repository task. Live probe results remain environment evidence rather than portable test assumptions: on the M4 review machine Pi was absent, Claude Code `2.1.104` was installed but logged out, and the first PATH Codex wrapper remained broken as previously recorded by `SPIKE-03`.
