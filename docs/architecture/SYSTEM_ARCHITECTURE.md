...
`manifest.json`, task snapshots, raw streams, traces, patches, grades, and
completed results are immutable once finalized. `status.json` is replaced
atomically and is never the sole evidence. Run IDs and artifact rules are
specified in [RUN_ARTIFACTS.md](RUN_ARTIFACTS.md).
